...
//           >
//             <div
//               // ADDED: The select-text class ensures the text is selectable.
//               className={`p-3 rounded-lg max-w-[80%] select-text ${ 
//                 msg.sender === 'user'
...
export default function Chatbot() {
  const [messages, setMessages] = useState<Message[]>([
    { sender: 'bot', text: 'Hello! I\'m your NexaInnov assistant. How can I help you today?' },
  ]);
  const [input, setInput] = useState<string>('');
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const messagesEndRef = useRef<HTMLDivElement | null>(null);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const botMsg: Message = { 
        sender: 'bot', 
        text: data.response || "I'm having trouble connecting to the brain. Try again?" 
      };
      setMessages((prev) => [...prev, botMsg]);
    } catch (error) {
      setMessages((prev) => [...prev, { sender: 'bot', text: 'Connection error. Please check your internet.' }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full md:h-[500px] w-full md:w-[350px] bg-[#12042A] rounded-lg shadow-lg border border-[#3A1F6E]">
      <div className="p-4 bg-[#1A0A3D] border-b border-[#3A1F6E] rounded-t-lg">
        <h2 className="text-lg font-semibold text-[#E9D5FF]">NexaInnov Support</h2>
      </div>
      
      <div className="flex-1 p-4 overflow-y-auto space-y-4">
        {messages.map((msg, index) => (
          <div key={index} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`p-3 rounded-lg max-w-[80%] select-text ${
              msg.sender === 'user' ? 'bg-[#6D28D9] text-white rounded-br-none' : 'bg-[#2E1058] text-[#E9D5FF] rounded-bl-none'
            }`}>
              {msg.text}
            </div>
          </div>
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="p-3 bg-[#2E1058] text-[#E9D5FF] rounded-lg animate-pulse">Typing...</div>
          </div>
        )}
        <div ref={messagesEndRef} />
      </div>
      ...
      <div className="p-4 border-t border-[#3A1F6E] rounded-b-lg">
        <div className="flex items-center space-x-2">
          <input
            ...
            onChange={(e) => setInput(e.target.value)}
            onKeyPress={handleKeyPress}
            className="flex-1 p-2 border border-[#3A1F6E] rounded-lg bg-[#1A0A3D] text-white focus:outline-none focus:ring-2 focus:ring-[#6D28D9]"
          ...
          <button
            ...
            className="p-2 bg-[#6D28D9] text-white rounded-lg disabled:opacity-50"
          >
            Send
          </button>
        </div>
